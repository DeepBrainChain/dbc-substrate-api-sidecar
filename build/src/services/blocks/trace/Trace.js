...
class Trace {
    ...
    registry) {
        ...
        if (!traceBlock.spans.length) {
            throw new http_errors_1.InternalServerError('No spans found. This runtime is likely not supported with tracing.');
        }
     ...
     */
    static extractExtrinsicIndex(spanIds, extrinsicIndexBySpanId) {
        ...
        return spanIds.reduce((uniqExtIdx, id) => {
            ...
            if (uniqExtIdx === undefined && extIdxToCheck) {
            ...
            else if (extIdxToCheck && !(uniqExtIdx === null || uniqExtIdx === void 0 ? void 0 : uniqExtIdx.eq(extIdxToCheck))) {
                ...
                throw new http_errors_1.InternalServerError('Expected at least one extrinsic index');
            }
            return uniqExtIdx;
        }, undefined);
    }
     ...
     */
    static findDescendants(root, spansById) {
        var _a, _b;
        // Note: traversal order doesn't matter here
        const stack = (_a = spansById.get(root)) === null || _a === void 0 ? void 0 : _a.children;
        if (!stack) {
            return [];
        }
        const descendants = [];
        while (stack.length) {
            ...
            const curSpanChildren = (_b = spansById.get(curId)) === null || _b === void 0 ? void 0 : _b.children;
            if (curSpanChildren) {
    ...
    static parseSpans(spans) {
        ...
        for (const span of spans) {
            ...
            if (span.name === SPANS.executeBlock.name && span.target === SPANS.executeBlock.target) {
                if (executeBlockSpanId !== undefined) {
                    throw new http_errors_1.InternalServerError('More than 1 execute block span found when only 1 was expected');
                }
        ...
        if (executeBlockSpanId === undefined) {
            throw new http_errors_1.InternalServerError('execute_block span could not be found');
        }
        ...
        for (const span of spans) {
            ...
            if (!maybeParentSpan) {
                throw new http_errors_1.InternalServerError('Expected spans parent to exist in spansById');
            }
    ...
    actionsAndOps() {
        ...
        for (const primary of spans) {
            ...
            if (primary.name === SPANS.applyExtrinsic.name) {
                ...
                secondarySpansIds.concat(primary.id), extrinsicIndexBySpanId);
                phase = types_1.Phase.ApplyExtrinsic;
            }
            else {
     ...
     * events.
     */
    deriveOperations(accountEventsByAddress) {
        const ops = [];
